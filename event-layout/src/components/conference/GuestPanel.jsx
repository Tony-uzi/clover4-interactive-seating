...
export default function GuestPanel({
  ...
  // Get unique group names for filtering (including 'all' option)
  // 确保 groups 和 guests 是数组
  const safeGroups = Array.isArray(groups) ? groups : [];
  const safeGuests = Array.isArray(guests) ? guests : [];
  const uniqueGroupNames = [...new Set(safeGroups.map(g => g.name))];
  const groupFilterOptions = ['all', ...uniqueGroupNames];
  ...
  const assignedCount = safeGuests.filter(g => g.tableNumber || g.elementId).length;
  const attendingCount = safeGuests.filter(g => g.attendance !== false).length;
  const checkedInCount = safeGuests.filter(g => g.checkedIn).length;
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {filteredGuests.length === 0 ? (
          ...
          filteredGuests.map((guest) => (
            <div
              key={guest.id}
              className="p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-400 transition-colors"
